...
static int read_packet(void *opaque, uint8_t *buf, int buf_size) {
    ...
    int r = (int) read(fdR, buf, (size_t) buf_size);
...
//    *input_codec_context = (*input_format_context)->streams[0]->codec;
    ...
    c = (*input_format_context)->streams[0]->codec;
    if (!(input_codec = avcodec_find_decoder(c->codec_id))) {
        fprintf(stderr, "Could not find input codec\n");
        avformat_close_input(input_format_context);
        return AVERROR_EXIT;
    }

    if ((error = avcodec_open2(c, input_codec, NULL)) < 0) {
        fprintf(stderr, "Could not open input codec (error '%s')\n",
                get_error_text(error));
        avformat_close_input(input_format_context);
        return error;
...
int main(int argc, char **argv) {
    ...
    const char *input_file_path = "/Users/chenweibin/reverseme.pcm";
    const char *output_file_path = "/Users/chenweibin/out.m4a";

    /** Register all codecs and formats so that they can be used. */
    av_register_all();
    /** Open the input file for reading. */
    //填充了输入格式上下文，并且打开了相对应的解码器
    if (open_input_file(input_file_path, &input_format_context,
                        &input_codec_context))
        goto cleanup;
