...
public:
    ...
    
    {
        int err = 0;
        {
            ...
            std::vector<std::string> forcegroundPages;
            
            if (beginEffect.length())
            {
                forcegroundPages.push_back(beginEffect);
            ...
            forcegroundPages.insert(forcegroundPages.end(), voicePages.begin(), voicePages.end());
            
            if (endEffect.length())
            {
                forcegroundPages.push_back(endEffect);
            ...
            for (std::string file : forcegroundPages)
            {
                int64_t duration = 0;
                ...
                forcegroundQueue.inputQueue.push_back(std::make_shared<AVProcessContext>(format, codec, (AVFilterContext*)NULL, streamIndex));
            }
            wholeDuration -= timeSpan;
            ...
            // init filter
            err = _configFilterGraphForCombineQueues(pool, forcegroundQueue, beginEffect.length(), endEffect.length(), timeSpan,
            ...
            inputQueues.push_back(forcegroundQueue);
