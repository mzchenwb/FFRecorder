...
int FFAudioBufferEncoder::beginInput()
{
    ...
    {
        // open output file
        err = openOutputFile(_outputFile, _context.format, _context.codec, _outputFileType, _outputBitrate);
        if (_context.format)
        {
            _pool.autoRelease([=] {
                if (_context.format->pb)
                    avio_closep(&_context.format->pb);
                avformat_free_context(_context.format);
            });
        }
        if (_context.codec)
        {
            _pool.autoRelease([=] {
                avcodec_close(_context.codec);
            });
        }
        AV_ERROR_CHECK(err);
        ...
        _context.filter = avfilter_graph_alloc_filter(graph, avfilter_get_by_name("abuffer"), NULL);
        ERROR_CHECKEX(_context.filter, err = AVERROR(ENOMEM));
        err = configInputFilter(_context.filter, AV_SAMPLE_FMT_S16, outputSampleRate, outputAudioChannelNum);
        AV_ERROR_CHECK(err);

        err = makeFormatForOutput(graph, _context.codec, _context.filter, _context.lastFilter);
        AV_ERROR_CHECK(err);
        err = makeOutput(graph, _context.codec, _context.lastFilter, _context.lastFilter);
        AV_ERROR_CHECK(err);
        ...
        err = avformat_write_header(_context.format, NULL);
        AV_ERROR_CHECK(err);
    }
...
int FFAudioBufferEncoder::appendData(const uint8_t* data, int len)
{
    ...
    int err = 0;
    {
        ...
        err = av_buffersrc_add_frame(_context.filter, frame);
        AV_ERROR_CHECK(err);
        ...
        do
        {
            ...
            err = av_buffersink_get_frame(_context.lastFilter, filteredFrame);
            if (err >= 0)
            {
                filteredFrame->pts = _framePts;
                _framePts += filteredFrame->nb_samples;

                err = encodeOneFrame(_context.format, _context.codec, filteredFrame, _packetPts);
                AV_ERROR_CHECK(err);
            }
        }
        while (err >= 0);
...
int FFAudioBufferEncoder::endInput()
{
    int err = 0;
    {
        err = av_buffersrc_add_frame(_context.filter, NULL);
        AV_ERROR_CHECK(err);

        err = encodeFlush(_context.format, _context.codec, _packetPts);
        AV_ERROR_CHECK(err);

        // write trailer
        err = av_write_trailer(_context.format);
        AV_ERROR_CHECK(err);
